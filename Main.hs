...
main = do
    ...
    let deepboi = (def :: BaseCreature) {
        ascores = AScores Abilities { str = 30, dex = 10, con = 29, int = 16, wis = 15, cha = 20 },
        hitDice = 25,
        size = Gargantuan,
        acKind = Natural 22,
        attacks = [
            ...
            Multiattack { names = ["Bite","Claw","Claw"] },
            def {
                name = "Scalding Breath",
                baseDmgRolls = [DmgRoll (18 `DTimes` (Die $ D 6)) Fire],
                atkKind = Special,
                target = Area $ Cone 60,
                save = SaveReduces { onPass = 0.5, granting = Con, saving = Dex},
                uses = Recharge 5 (D 6)
